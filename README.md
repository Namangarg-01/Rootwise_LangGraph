...
- **Math renders as actual LaTeX, not raw text.** Despite an explicit prompt instruction, models reliably default to `\( \)` / `\[ \]` (their strongest LaTeX training prior) instead of the `$...$` / `$$...$$` Streamlit's KaTeX renderer requires — every response is normalized before display, including a safety net for the rare case where a model mixes delimiter styles within one equation. A second, separate failure mode is also handled: the model sometimes writes a multi-step `&`-aligned derivation closed with `\end{aligned}` but forgets the matching `\begin{aligned}` — perfectly balanced `$$` delimiters, but invalid LaTeX inside them, which KaTeX renders as a wall of red error text. The missing `\begin{aligned}` is inserted automatically.
